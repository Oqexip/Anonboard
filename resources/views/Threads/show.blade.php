...
<x-app-layout :title="$thread->title ?? 'Thread'">
    @php
        // Warna nama berdasarkan user / anon
    ...
    {{-- THREAD CARD (angkat z saat menuOpen=true) --}}
    <article x-data="{ openEditThread: false, menuOpen: false }" :class="menuOpen ? 'relative z-50' : 'relative z-10'"
        class="bg-white/80 backdrop-blur rounded-2xl shadow-sm border border-slate-200 p-6">

        <header class="mb-4 flex items-start justify-between gap-4">
            <div class="space-y-1">
                <h1 class="text-2xl font-bold tracking-tight">{{ $thread->title ?? 'Thread' }}</h1>
                <div class="text-sm text-slate-600 flex items-center gap-2">
                    ...
                    <time
                        datetime="{{ $thread->created_at->toIso8601String() }}">{{ $thread->created_at->diffForHumans() }}</time>

                    @if ($thread->edited_at)
                        <span class="text-slate-400 text-xs"
                            title="Last edited {{ $thread->edited_at->diffForHumans() }}">(edited)</span>
                    @endif
                </div>
            </div>
            ...
            <div class="relative" @keydown.escape.window="menuOpen=false" @click.outside="menuOpen=false">
                ...
                <div x-show="menuOpen" x-cloak
                    ...
                    @if ($canEditThread)
                        <button type="button" @click="menuOpen=false; openEditThread = !openEditThread"
                            class="w-full flex items-center gap-2 px-3 py-2 text-sm hover:bg-slate-50">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-slate-600" fill="currentColor"
                                viewBox="0 0 24 24">
                    ...
                    <button type="button"
                        ...
                        class="w-full flex items-center gap-2 px-3 py-2 text-sm hover:bg-slate-50">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-slate-600" viewBox="0 0 24 24"
                            fill="currentColor">
                            <path d="M8 13h8v-2H8v2z" />
                            <path
                                d="M3.9 12a3.9 3.9 0 013.9-3.9h2v1.8h-2a2.1 2.1 0 000 4.2h2v1.8h-2A3.9 3.9 0 013.9 12zM16.2 8.1h-2v1.8h2a2.1 2.1 0 010 4.2h-2v1.8h2a3.9 3.9 0 000-7.8z" />
                        </svg>
                        Save
                    </button>
        ...
        @if ($canEditThread)
            <form x-show="openEditThread" x-cloak method="POST" action="{{ route('threads.update', $thread) }}"
                ...
                <textarea name="content" rows="4"
                    class="w-full rounded-xl border-slate-300 focus:border-sky-500 focus:ring-sky-500" required>{{ old('content', $thread->content) }}</textarea>
    ...
    @if (!$thread->is_locked)
        <section class="bg-white/80 backdrop-blur rounded-2xl border border-slate-200 shadow-sm p-6 mt-6">
            <form action="{{ route('comments.store', $thread) }}" method="POST" enctype="multipart/form-data"
                ...
                @csrf
                <textarea name="content" rows="4"
                    class="w-full rounded-xl border-slate-300 focus:border-sky-500 focus:ring-sky-500" placeholder="Write a comment"
                    required></textarea>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <label for="images-input"
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 24 24"
                                fill="currentColor">
                                <path
                                    d="M20 6h-3.586l-1.707-1.707A.996.996 0 0 0 14 4h-4L7.586 6H4c-1.103 0-2 .897-2 2v10a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V8c0-1.103-.897-2-2-2zM12 19a5 5 0 1 1 0-10 5 5 0 0 1 0 10z" />
                            </svg>
