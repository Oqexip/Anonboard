...
<div id="c-{{ $c->id }}" class="relative pl-4 sm:pl-6"
    ...
    <div class="rounded-lg bg-white shadow-sm ring-1 ring-gray-200/60 p-3" x-data="commentItem({
        id: {{ $c->id }},
        score: {{ (int) $c->score }},
        csrf: '{{ csrf_token() }}',
        voteUrl: '{{ url('/vote') }}',
        openEdit: false
    })" x-init="$refs.score && ($refs.score.textContent = score)">
        ...
        <div class="text-sm text-gray-600 mb-2 flex items-center gap-2">
            ...
            {{-- ACTION MENU --}}
            <div class="ml-auto relative" x-data="{ open: false }" @keydown.escape.window="open=false"
                ...
                <div x-show="open" x-cloak
                    ...
                    @if ($canEditComment)
                        <button type="button" @click="open=false; openEdit = !openEdit"
                    ...
                    <button type="button"
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
                            <path
                                d="M3.9 12a3.9 3.9 0 013.9-3.9h2v1.8h-2a2.1 2.1 0 000 4.2h2v1.8h-2A3.9 3.9 0 013.9 12zm12.3-3.9h-2v1.8h2a2.1 2.1 0 010 4.2h-2v1.8h2a3.9 3.9 0 000-7.8z" />
                    ...
                    @can('delete', $c)
                        <form method="POST" action="{{ route('comments.destroy', $c) }}"
                            ...
                            @csrf @method('DELETE')
                            <button
                                class="w-full px-3 py-2 text-left text-sm hover:bg-rose-50 text-rose-600 flex items-center gap-2"
                                role="menuitem" tabindex="-1">
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" viewBox="0 0 24 24"
                                    fill="currentColor">
        ...
        <div class="mb-2">
                    ...
                    <div class="mt-2 grid grid-cols-2 gap-2">
                        @foreach ($c->attachments as $a)
                            ...
                            <a href="{{ $url }}" target="_blank" class="block">
                                <img src="{{ $url }}" alt=""
                                    class="rounded-md max-h-40 object-cover w-full" loading="lazy" decoding="async">
                            </a>
                        @endforeach
                    </div>
                @endif
            ...
            @if ($canEditComment)
                <form x-show="openEdit" x-cloak method="POST" action="{{ route('comments.update', $c) }}"
                    class="space-y-2">
                    @csrf @method('PATCH')
                    ...
                    <div class="flex items-center gap-2">
                        <button
                            class="px-3 py-1.5 rounded-lg bg-emerald-600 text-white hover:bg-emerald-700">Save</button>
                        <button type="button" class="px-3 py-1.5 rounded-lg bg-gray-100 hover:bg-gray-200"
        ...
        @if (!$thread->is_locked && $c->depth < 5)
            <form action="{{ route('comments.store', $thread) }}" method="POST" enctype="multipart/form-data"
                ...
                <textarea name="content" rows="3"
                    class="w-full rounded-md border-gray-300 focus:border-indigo-500 focus:ring-indigo-500" placeholder="Reply..."
                    required></textarea>
                ...
                <div class="flex items-center justify-between gap-2 flex-nowrap">
                    ...
                    <div class="flex items-center gap-2 min-w-0">
                        ...
                        <label for="{{ $inputId }}"
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 24 24"
                                fill="currentColor">
                                <path
                                    d="M20 6h-3.586l-1.707-1.707A.996.996 0 0 0 14 4h-4c-.265 0-.52.105-.707.293L7.586 6H4c-1.103 0-2 .897-2 2v10a2 2 0 0 0 2 2h16c1.103 0 2-.897 2-2V8c0-1.103-.897-2-2-2zM12 19a5 5 0 1 1 0-10 5 5 0 0 1 0 10z" />
                            </svg>
                    ...
                    {{-- Kanan: tombol post (jangan melebar) --}}
                    <button type="submit"
                ...
                async vote(val) {
                    try {
                        ...
                        if (typeof data.score !== 'undefined') {
                            ...
                            this.$refs.score && (this.$refs.score.textContent = data.score);
                        }
                    } catch (e) {
